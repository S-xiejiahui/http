...
const char *Judge_Method(char *method, int Socket) {
  ...
  if (strcmp(method, "GET") == 0) {
    ...
    clienterror(Socket, method, "501", "Not Implemented",
                "XJH does not implement thid method");
    return ERROR;
  }
}
/****************************************************
...
const char *Post_Value(char *message, int Socket) {
  ...
  count += snprintf(body + count, sizeof(body) - count,
                    "Server: XJH Web Server\r\n");
  ...
  if (Judge_Method(method, fd) == ERROR) {
    ...
    if (is_static) { //服务静态内容
      ...
      if (!(S_ISREG(sbuf.st_mode)) || !(S_IXUSR & sbuf.st_mode)) {
        clienterror(fd, filename, "403", "Forbidden",
                    "XJH couldn't run the CGI program");
        return;
      }
...
void show_picture(int fd, char *index) {
  int count = 0;
  char body[MAXBUF], file[MAXLINE], send_filename[64];
  ...
  count +=
      snprintf(body + count, sizeof(body), "Content-length: %d\r\n", pic_size);
  count +=
      snprintf(body + count, sizeof(body), "Content-type: %s\r\n\r\n", "jpg");
...
void serve_static(int fd, char *filename, int filesize) {
  ...
  int count = 0;
  char *srcp = NULL;
  char body[MAXBUF], filetype[MAXLINE];
...
void clienterror(int fd, char *cause, char *errnum, char *shortmsg,
  ...
  count +=
      snprintf(body + count, sizeof(body), "<html><title>XJH Error</title>");
  count += snprintf(body + count, sizeof(body), "<body bgcolor=\"white\">\r\n");
  count += snprintf(body + count, sizeof(body),
                    "<center><h1>%s: %s</h1></center>", errnum, shortmsg);
  count += snprintf(body + count, sizeof(body),
                    "<center><h3>%s: %s</h3></center>", longmsg, cause);
  count += snprintf(body + count, sizeof(body),
                    "<hr><center>The XJH Web server</center>\r\n");
